...
import { getAllDeployments } from '@/routes/api/deployments'
import type { Deployment } from '@/types'

export const Route = createFileRoute('/deployments')({
	component: DeploymentsPage,
	loader: async () => await getAllDeployments(),
})

function DeploymentsPage() {
	const { items: deployments } = Route.useLoaderData()
			...
			<div className="mx-auto max-w-7xl min-w-fit px-4 pb-20 md:px-8">
				...
				<div className="border-2 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
						...
						<table className="w-full min-w-[600px] border-collapse text-left">
							<thead>
								<tr className="border-b-2 border-black bg-neutral-100 text-xs font-black text-black">
									<th className="p-4 tracking-wider uppercase">Project</th>
					...
					<div className="flex items-center justify-between border-t-2 border-black bg-neutral-50 p-4 font-mono text-xs font-bold">
						...
						<div className="flex gap-2">
							<button className="border-2 border-black bg-white px-4 py-1 transition-colors hover:bg-black hover:text-white disabled:opacity-50">
								PREV
							</button>
							<button className="border-2 border-black bg-white px-4 py-1 transition-colors hover:bg-black hover:text-white">
								NEXT
							</button>
						</div>
					</div>
...
function DeploymentRow({ deployment }: { deployment: Deployment }) {
	...
	return (
		<tr className="group transition-colors hover:bg-neutral-50">
			{/* Project - Status + Avatar + Name + Badge */}
			<td className="px-4 py-3">
				<div className="flex items-center gap-3">
					...
					{/* Avatar */}
					<div
						className="flex size-8 shrink-0 items-center justify-center border-2 border-black text-xs font-bold text-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-colors"
						style={{
							backgroundColor: getColorFromString(deployment.projectName ?? ''),
						}}
					>
						{deployment.projectName?.[0]?.toUpperCase() ?? '?'}
					</div>
			...
			{/* Commit - Branch + SHA + Message + Author */}
			<td className="px-4 py-3">
