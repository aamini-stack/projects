...
test("Block after limit is reached", () => {
  ...
  jest.advanceTimersByTime(15 * 60 * 1000 + 1); // Fast forward 15 minutes and 1 second
  expect(limiter.consume(TEST_IP)).toEqual({ success: true, remaining: 0 });
});
...
test("Don't fill tokens before window passes", () => {
  expect(limiter.consume(TEST_IP)).toEqual({ success: true, remaining: 2 });
  jest.advanceTimersByTime(14 * 60 * 1000); // Fast forward 14 minutes (less than window)
  expect(limiter.consume(TEST_IP)).toEqual({ success: true, remaining: 1 });
});

test("Multiple windows passing", () => {
  expect(limiter.consume(TEST_IP)).toEqual({ success: true, remaining: 2 });
  jest.advanceTimersByTime(45 * 60 * 1000); // Fast forward 45 minutes (3 windows)
  expect(limiter.consume(TEST_IP)).toEqual({ success: true, remaining: 2 });
});

test("Block after global limit of 100 is reached", () => {
  for (let i = 0; i < 100; i++) {
    expect(limiter.consume(`192.168.1.${i}`)).toEqual({
      success: true,
      remaining: 2,
    });
  }
  ...
  jest.advanceTimersByTime(65 * 60 * 1000); // Fast forward 65 minutes (1.0833 windows)
  expect(limiter.consume(TEST_IP)).toEqual({ success: true, remaining: 2 });
});
