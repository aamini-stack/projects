...
**Goal:** Clean up and modernize the @scripts/ folder by removing dead code, consolidating actions into chunky modules, and adding unit tests for business logic.
...
export async function githubRequest<T>(input: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`GitHub API ${response.status} for ${input.path}: ${details || response.statusText}`)
  }
...
export async function createDeploymentStatus(input: {
  repo: RepoRef
  deploymentId: number
  state: 'queued' | 'in_progress' | 'success' | 'failure' | 'pending' | 'error' | 'inactive'
  description: string
...
function parseCreatePreviewArgs(args: string[]): CreatePreviewOptions {
  ...
  if (!values.repository || !values.sha || !values['run-url'] || !values['workflow-ref']) {
...
export async function createPreviews(rawArgs: string[]): Promise<void> {
  ...
  const changedFiles = await listPullRequestFiles({ prNumber: options.prNumber, repo })
  ...
  for (const app of requiredApps) {
    ...
    const deploymentId = await createDeployment({
      environment,
      payload: { app, environment, image_tag: imageTag, pr_number: options.prNumber, sha: options.sha, url },
      repo,
      sha: options.sha,
      task: 'preview-app',
    })
    ...
    await createWorkflowDispatch({
      ...
      inputs: { app, environment: 'preview', url, sha: options.sha, pr_number: String(options.prNumber), image_tag: imageTag },
    })
...
function parseCleanupArgs(args: string[]): CleanupOptions {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    const value = args[index + 1]
    if (!key?.startsWith('--') || value === undefined) {
      throw new Error('Usage: preview cleanup --repository <owner/repo> --event-name <name> --event-path <path> --run-url <url>')
    }
    values[key.slice(2)] = value
  }

  if (!values.repository || !values['event-name'] || !values['event-path'] || !values['run-url']) {
    throw new Error('Missing required arguments')
  }
...
function parseEventInput(input: { event: PullRequestEvent; eventName: string }): { prNumber: number | null; shouldCleanup: boolean } {
...
export async function cleanupPreviews(rawArgs: string[]): Promise<void> {
  ...
  const event = JSON.parse(readFileSync(options.eventPath, 'utf8')) as PullRequestEvent
...
function parseStatusArgs(args: string[]): StatusOptions {
  ...
  if (!values.repository || !values.sha || !values.app || !values.url || !values['run-url']) {
    throw new Error('Missing required arguments')
  }
...
async function resolveDeploymentId(options: StatusOptions): Promise<number | null> {
  if (options.deploymentId) return options.deploymentId
  if (options.environmentType !== 'preview' || !options.deploymentEnvironment) return null

  const repo = parseRepo(options.repository)
  const deployments = await listDeployments({ environment: options.deploymentEnvironment, repo, sha: options.sha })
...
export async function updatePreviewStatus(rawArgs: string[]): Promise<void> {
  ...
  if (!deploymentId) {
    console.log(`No deployment record resolved for ${options.app}; skipping status update.`)
    return
  }

  const state = options.command === 'mark-in-progress' ? 'in_progress' : options.result!
  const description =
...
function parseGateArgs(args: string[]): GateOptions {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    const value = args[index + 1]
    if (!key?.startsWith('--') || value === undefined) {
      throw new Error('Usage: preview gate --repository <owner/repo> --event-name <name> --event-path <path> --run-url <url> --timeout-seconds <seconds>')
    }
    values[key.slice(2)] = value
  }
...
function parseGateEvent(input: { event: DeploymentEvent; eventName: string }): { prNumber: number | null; sha: string | null; skip: boolean } {
  if (input.eventName === 'deployment_status') {
    ...
    return match ? { prNumber: Number(match[1]), sha, skip: false } : { prNumber: null, sha, skip: false }
  }
...
export async function runPreviewGate(rawArgs: string[]): Promise<void> {
  ...
  const event = JSON.parse(readFileSync(options.eventPath, 'utf8')) as DeploymentEvent
  ...
  const repoRoot = await getRepoRoot()
  const allApps = listAppDirectories(repoRoot).sort()
  const changedFiles = await listPullRequestFiles({ prNumber: parsed.prNumber, repo })
  const requiredApps = selectRequiredApps({ allApps, changedFiles })
  ...
  for (const app of requiredApps) {
    ...
    const deployments = await listDeployments({ environment, repo, sha: parsed.sha })
    const deployment = deployments.filter((d) => d.task === 'preview-app').sort((l, r) => l.created_at.localeCompare(r.created_at)).pop()
    ...
    const latestStatus = (await listDeploymentStatuses({ deploymentId: deployment.id, repo }))[0]?.state
    ...
    if (['queued', 'in_progress', 'pending', 'missing'].includes(deploymentState)) {
      gateState = 'pending'
      pendingDetails.push(`${app}:${deploymentState}`)
      const createdEpoch = Date.parse(deployment.created_at)
      if (Number.isFinite(createdEpoch) && (oldestPendingEpoch === null || createdEpoch < oldestPendingEpoch)) {
  ...
  if (gateState === 'pending') {
    ...
    if (oldestPendingEpoch !== null) {
      const pendingAgeSeconds = Math.floor((Date.now() - oldestPendingEpoch) / 1000)
  ...
  console.log(`Published deployments/preview-gate=${gateState} for sha ${parsed.sha}.`)
}
...
- Delete: `scripts/src/actions/e2e-commit-status.ts`, `scripts/src/actions/e2e-required-apps.ts`
...
export function selectE2ERequiredApps(input: SelectE2ERequiredAppsInput): string[] {
  const allApps = [...input.allApps].sort()
...
function parseStatusArgs(args: string[]): StatusOptions {
  ...
  if (!values.repository || !values.sha || !values.app || !values.source || !values['run-url']) {
    throw new Error('Missing required arguments')
  }
...
**Files:**
- Create: `scripts/src/events/index.ts` (consolidate deploy-ready.ts + deploy-ready-outputs.ts)
...
export function normalizeDeployReadyEvent(event: GitHubDispatchEnvelope): NormalizedDeployReadyEvent {
  ...
  return {
    ...
    source:
      ...
      metadata.source ??
      (event.deployment_status ? 'github-deployment-status' : 'flux-deploy-ready'),
    url,
  }
}

function deriveEnvironmentFromDeploymentName(value: string | undefined): 'preview' | 'stable' | undefined {
...
function derivePrNumberFromDeploymentName(value: string | undefined): string | null {
  if (!value) return null
...
function derivePreviewImageTag(app: string, name?: string, changeRequest?: string): string {
  const prNumber =
    changeRequest ??
...
function normalizeDeploymentId(value: number | string | undefined): number | null {
  if (value === undefined || value === '') return null
  const parsed = Number(value)
  if (!Number.isInteger(parsed) || parsed < 1) {
...
function parseArgs(args: string[]): OutputsOptions {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    const value = args[index + 1]
    if (!key?.startsWith('--') || value === undefined) {
      throw new Error('Usage: events outputs --event-path <path> --github-output <path>')
    }
    values[key.slice(2)] = value
  }
...
async function main(): Promise<void> {
  ...
  if (subcommand === 'outputs') {
    ...
    const event = JSON.parse(readFileSync(rawArgs[0], 'utf8')) as GitHubDispatchEnvelope
...
describe('selectRequiredApps', () => {
  it('returns all apps if no files changed', () => {
    expect(selectRequiredApps({ allApps: ['a', 'b'], changedFiles: [] })).toEqual(['a', 'b'])
  })

  it('returns all apps if file outside apps/ changed', () => {
    expect(selectRequiredApps({ allApps: ['a', 'b'], changedFiles: ['packages/utils/index.ts'] })).toEqual(['a', 'b'])
  })

  it('returns only affected apps', () => {
    expect(selectRequiredApps({ allApps: ['a', 'b', 'c'], changedFiles: ['apps/a/src/index.ts'] })).toEqual(['a'])
  })

  it('sorts results alphabetically', () => {
    expect(selectRequiredApps({ allApps: ['c', 'a', 'b'], changedFiles: ['apps/c/src/index.ts', 'apps/a/src/index.ts'] })).toEqual(['a', 'c'])
  })
})
```
...
describe('selectE2ERequiredApps', () => {
  it('returns all apps for flux-deploy-ready without PR', () => {
    expect(selectE2ERequiredApps({ allApps: ['a', 'b'], app: 'x', changedFiles: [], prNumber: null, source: 'flux-deploy-ready' })).toEqual(['a', 'b'])
  })

  it('returns single app for non-flux source without PR', () => {
    expect(selectE2ERequiredApps({ allApps: ['a', 'b'], app: 'x', changedFiles: [], prNumber: null, source: 'other' })).toEqual(['x'])
  })

  it('returns affected apps for PR', () => {
    expect(selectE2ERequiredApps({ allApps: ['a', 'b', 'c'], app: 'x', changedFiles: ['apps/a/src/index.ts'], prNumber: 123, source: 'github' })).toEqual(['a'])
  })
})
```
